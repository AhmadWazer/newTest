...
$sql = "SELECT class_1.classid, class_1.class_name FROM class_1";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>';
        echo '<th scope="row">' . $row['classid'] . '</th>';
        echo '<td>' . $row['class_name'] . '</td>';
        echo '<td style="width:500px">';
        echo '<a href="delet_class.php?id='.$row['classid'].' " onclick="copyValue(this)" class="btn btn btn-danger edit-btn m-2" >Delete</a>';
        echo '<a href="update.php?id='.$row['classid'].' " onclick="copyValue(this)" class="btn btn btn-success edit-btn m-2" >Edit</a>';
        echo '</td>';
        echo '</tr>';
    }
